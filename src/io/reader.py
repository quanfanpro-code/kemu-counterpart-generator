...
def detect_ledger_account_column(columns: List[str]) -> Optional[str]:
    ...
    for keyword in LEDGER_ACCOUNT_KEYWORDS:
        ...
        # 只做"关键词包含于列名"的单向匹配，避免"公司"这类短列名被误判为账套列
...
def _normalize_amount_columns(df: pd.DataFrame) -> Optional[pd.DataFrame]:
    ...
    for col in AMOUNT_COLUMNS:
        ...
        if 非法值掩码.any():
            # 用位置索引定位，避免索引标签与位置不一致时取错行
            非法位置 = 非法值掩码.to_numpy().nonzero()[0][:5]
            非法样例 = [f"第{位置 + 2}行={原始值.iloc[位置]!r}" for 位置 in 非法位置]
            logger.error(f"错误: [{col}] 列存在无法识别的金额值：{'; '.join(非法样例)}")
            return None
    ...
    for col in cols_to_ffill:
        if col in df.columns:
            ...
            if df[col].isnull().any():
                if col == '凭证编号':
                    logger.warning(
                        f"警告：[{col}] 列存在空值，已向下填充（视为合并单元格）。"
                        "若空值并非合并单元格所致，这些行将被并入上一张凭证，请确认原始数据！"
                    )
                else:
