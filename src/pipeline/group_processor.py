...
class GroupProcessor:
    ...
    def process(self) -> Tuple[List[Dict[str, Any]], List[str]]:
        ...
        if self._process_profit_loss():
            return self.output_rows, self.warnings
    ...
    def _prepare_data(self):
        # zip 保留原始行号(_orig_idx)，供主进程恢复原始行序
        for orig_idx, row in zip(self.group.index, self.group.to_dict('records')):
            ...
            if d_nonzero and c_nonzero:
                # 借贷同行为异常数据：拆成借、贷两个独立条目分别参与匹配，
                # 避免贷方金额被静默丢弃导致合计不符
                self.warnings.append(
                    f"⚠️ 数据异常：分组{self.name} 科目「{subject}」借贷双方同时存在非零金额"
                    f"（借方={d}, 贷方={c}），已拆分为借/贷两条分别匹配。"
                )
                item_d = dict(item)
                item_d['row_data'] = row.copy()
                item_d['row_data']['贷方发生额'] = 0
                item_d['amount'] = float(PrecisionEngine.to_decimal(d))
                item_d['amount_li'] = PrecisionEngine.to_integer_li(d)
                item_d['side'] = 'debit'
                self.debit_items.append(item_d)
                item_c = dict(item)
                item_c['row_data'] = row.copy()
                item_c['row_data']['借方发生额'] = 0
                item_c['amount'] = float(PrecisionEngine.to_decimal(c))
                item_c['amount_li'] = PrecisionEngine.to_integer_li(c)
                item_c['side'] = 'credit'
                self.credit_items.append(item_c)
                continue
    ...
    def _match_standard_rules(self):
        ...
        for d in self.debit_items:
            ...
            for c in self.credit_items:
                if c['matched'] or c['row_id'] == d['row_id']: continue
        ...
        for d in self.debit_items:
            ...
            candidates = [(i, c) for i, c in enumerate(self.credit_items)
                          if not c['matched'] and c['row_id'] != d['row_id']
                          and PrecisionEngine.compare_amounts(c['amount'], d['amount']) <= 0
        ...
        for c in self.credit_items:
            ...
            candidates = [(i, d) for i, d in enumerate(self.debit_items)
                          if not d['matched'] and d['row_id'] != c['row_id']
                          and PrecisionEngine.compare_amounts(d['amount'], c['amount']) <= 0
                          and check_rule_match(d['subject'], c['subject'])]
    ...
    def _match_fallback(self):
        ...
        while d_idx < len(unmatched_debit) and c_idx < len(unmatched_credit):
            ...
            if d_item['row_id'] == c_item['row_id']:
                # 同行拆出的借/贷条目不允许互相配对：从后面换一个不同行的贷方上来；
                # 换不到就结束循环，双方都会落入下方"异常剩余"，金额不丢
                swap_j = next((j for j in range(c_idx + 1, len(unmatched_credit))
                               if unmatched_credit[j]['row_id'] != d_item['row_id']), None)
                if swap_j is None:
                    break
                unmatched_credit[c_idx], unmatched_credit[swap_j] = \
                    unmatched_credit[swap_j], unmatched_credit[c_idx]
                c_item = unmatched_credit[c_idx]
